...
import { Mail, MapPin, Clock, CheckCircle, Send, MessageSquare, Github, Linkedin, Twitter } from 'lucide-react'

interface FormData { name: string; email: string; course: string; message: string }

const courseOptions = ['MERN Stack Bootcamp','Full Stack Developer','React Masterclass','Backend Engineering','Web3 Development','AI for Developers','Not sure yet']
...
export default function ContactPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Fallback: open Gmail compose with the message
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="lg:col-span-2 space-y-5">
            <div className="card p-6 bg-gradient-to-br from-primary/10 to-accent-cyan/5 border-primary/30">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <div className="text-xs text-primary font-semibold">Founder & Lead Instructor</div>
              ...
              <div className="flex gap-2">
                {[{ icon: <Github className="w-4 h-4"/>, href:'https://github.com' },{ icon:<Linkedin className="w-4 h-4"/>, href:'https://linkedin.com' },{ icon:<Twitter className="w-4 h-4"/>, href:'https://twitter.com' }].map((s,i)=>(
            ...
            {[
              ...
              { icon:<MapPin className="w-5 h-5"/>, label:'Location', value:'Purulia, West Bengal, 723128, India', sub:'IST (UTC +5:30)', color:'text-accent-emerald bg-accent-emerald/10 border-accent-emerald/25', href:null },
              ...
              <motion.div key={i} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:0.1+i*0.08}} className="card p-4 flex gap-4">
                ...
                <div>
                  ...
                  {item.href ? <a href={item.href} className="font-semibold text-sm text-text-primary hover:text-primary transition-colors">{item.value}</a> : <p className="font-semibold text-sm text-text-primary">{item.value}</p>}
            ...
            <div className="card p-8 lg:p-10">
              {submitted ? (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-12">
                  ...
                  <p className="text-text-secondary mb-2 text-sm max-w-sm mx-auto">Subhadip will reply to your email within 2 hours. You should also receive an auto-reply shortly.</p>
                  <p className="text-xs text-text-muted mb-6">Check <span className="text-primary">subhadipmahanty@gmail.com</span> for the admin copy.</p>
                ...
                <>
                  ...
                  <p className="text-sm text-text-secondary mb-8">You will receive an auto-reply instantly and a personal reply within 2 hours from <span className="text-primary">subhadipmahanty@gmail.com</span>.</p>
                  {error && <div className="mb-5 p-3 rounded-xl bg-accent-rose/10 border border-accent-rose/25 text-sm text-accent-rose">{error}</div>}
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      <div>
                        ...
                        <input {...register('name',{required:'Name is required',minLength:{value:2,message:'Min 2 chars'}})} placeholder="Your full name" className={`w-full px-4 py-3 rounded-xl bg-elevated border text-text-primary placeholder:text-text-muted text-sm focus:outline-none transition-colors ${errors.name?'border-accent-rose/60':'border-border focus:border-primary/60'}`}/>
                    ...
                    <div>
                      ...
                      <textarea {...register('message',{required:'Message is required',minLength:{value:10,message:'At least 10 characters'}})} rows={5} placeholder="Tell me about your background, your goals, or any questions about the courses..." className={`w-full px-4 py-3 rounded-xl bg-elevated border text-text-primary placeholder:text-text-muted text-sm focus:outline-none transition-colors resize-none ${errors.message?'border-accent-rose/60':'border-border focus:border-primary/60'}`}/>
                    ...
                    <p className="text-xs text-text-muted text-center">Message goes directly to <span className="text-text-secondary font-medium">subhadipmahanty@gmail.com</span></p>
                  </form>
                </>
              )}
            </div>
